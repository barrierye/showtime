...
class WebSpider(object):
    ...
    def _get_default_header(self, browser_type='Chrome'):
        ...
        return header
    async def async_get_page_by_GET(self, url, index, page_lsit):
    ...
    def get_page_by_GET(self, url):
        return self._session.get(url, headers=self._get_default_header('IE')).content.decode('utf-8')
    ...
    def _async_get_rough_page(self, url, index, page_lsit):
        ''' 获取给定url（从self._get_rough_url_list的搭配的url）的内容，default是直接GET '''
        return self.async_get_page_by_GET(url, index, page_lsit)
    ...
    def _get_detailed_page(self, url):
        ...
        return self.get_page_by_GET(url)
    def _async_get_detailed_page(self, url, index, page_lsit):
        ''' 获取给定url（包含演出详细信息的url）的内容，default是直接GET '''
        return self.async_get_page_by_GET(url, index, page_lsit)
    ...
    def get_show_list(self, is_parallel=True, process_num=None):
        ...
        tasks = [self._async_get_rough_page(url, index, page_lsit) for index, url in enumerate(urls)]
        loop.run_until_complete(asyncio.wait(tasks))

        rough_show_infos = []
        for page in page_lsit:
            rough_show_infos += self._parse_for_rough_info(page[1])

        showlist.add_shows([show_type.Show(info) for info in rough_show_infos])

        # get detailed infos
        page_lsit = []
        loop = asyncio.get_event_loop() # only one instance
        tasks = [self._async_get_detailed_page(show.url, index, page_lsit) for index, show in enumerate(showlist)]
        loop.run_until_complete(asyncio.wait(tasks))
        #  loop.close() # can not close because loop is the only one instance
        page_lsit.sort(key=lambda e: e[0])

        for i, show in enumerate(showlist):
            showlist[i].add_events(self._parse_for_detailed_info(page_lsit[i][1]))
        return showlist
...
class BeihangSunriseConcertHall(WebSpider):
    ...
    def _parse_for_detailed_info(self, text):
        detailed_info_regex = re.compile(
                ...
                r'(?P<place>.*?)</span>.*?【票价】(?P<total_price>.*?)</span>', re.DOTALL)
        regex_res = detailed_info_regex.findall(text)
